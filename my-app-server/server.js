...
app.post('/api/buyers', (req, res) => {
    const { first_name, last_name,name, phone_number, email, address, province, postal_code, user_id } = req.body;
    ...
    // Generate buyer_id automatically
    const getLastBuyerIdQuery = 'SELECT buyer_id FROM buyer ORDER BY buyer_id DESC LIMIT 1';

    pool.query(getLastBuyerIdQuery, (error, results) => {
        if (error) {
            console.error('Error retrieving last buyer_id:', error.message);
            return res.status(500).json({ error: 'Error retrieving last buyer_id' });
        }

        let newBuyerId = 'B0001'; // Default value if no records exist
        if (results.length > 0) {
            const lastBuyerId = results[0].buyer_id;
            const lastNumericPart = parseInt(lastBuyerId.substring(1)); // Remove "B" and convert to number
            const nextNumericPart = lastNumericPart + 1;
            newBuyerId = 'B' + nextNumericPart.toString().padStart(4, '0'); // Generate new ID as BXXXX
        }

        // Insert buyer data into the database
        const query = `
            INSERT INTO buyer (buyer_id, first_name, last_name,name, phone_number, email, address, province, postal_code, user_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)
        `;

        pool.query(query, [newBuyerId, first_name, last_name,name, phone_number, email, address, province, postal_code, user_id], (error, results) => {
            if (error) {
                console.error('Error inserting buyer data:', error.message);
                return res.status(500).json({ error: 'Error inserting buyer data' });
            }
            res.status(201).json({ message: 'Buyer data saved successfully', buyer_id: newBuyerId });
        });
    });
});


// // API สำหรับเรียกข้อมูลผู้รับซื้อ
// app.get('/api/buyers', (req, res) => {
//     const sql = 'SELECT * FROM buyer';
//     db.query(sql, (error, results) => {
//       if (error) {
//         return res.status(500).json({ message: 'Error fetching buyer data', error });
//       }
//       res.status(200).json(results);
//     });
//   });
...
app.post('/api/farmers', (req, res) => {
    ...
    // สร้างรหัส farmer_id อัตโนมัติ
    const getLastFarmerIdQuery = 'SELECT farmer_id FROM farmers ORDER BY farmer_id DESC LIMIT 1';

    pool.query(getLastFarmerIdQuery, (error, results) => {
        if (error) {
            console.error('Error retrieving last farmer_id:', error);
            return res.status(500).json({ error: 'Error retrieving last farmer_id' });
        }

        let newFarmerId = 'F0001'; // ค่าเริ่มต้นถ้าไม่มีข้อมูลในตาราง
        if (results.length > 0) {
            const lastFarmerId = results[0].farmer_id;
            const lastNumericPart = parseInt(lastFarmerId.substring(1)); // ตัดตัวอักษร "F" ออกแล้วแปลงเป็นตัวเลข
            const nextNumericPart = lastNumericPart + 1;
            newFarmerId = 'F' + nextNumericPart.toString().padStart(4, '0'); // สร้างรหัสใหม่ตามรูปแบบ FXXXX
        }
        ...
        pool.query(query, [newFarmerId, first_name, last_name, name, phone_number, email, address, province, postal_code, user_id], (error, results) => {
            if (error) {
                console.error('Error inserting farmer data:', error);
                return res.status(500).json({ error: 'Error inserting farmer data' });
            }
            res.status(201).json({ message: 'Farmer data saved successfully', farmer_id: newFarmerId });
        });
    });
});
...
app.get('/api/history', (req, res) => {
    ...
    console.log(`Fetching history for farmerId: ${farmerId}`); // Log ค่าที่ได้รับมา
    ...
    pool.query(query, [farmerId], (err, result) => {
        ...
        console.log('Fetched data from DB:', result); // Log ข้อมูลที่ query มาได้
        res.json(result);
    });
});
